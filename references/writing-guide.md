...
### ⛔ 微信兼容性禁止项（converter 不支持，写了会渲染出错）

**1. 禁止使用 GitHub 风格 Alert 语法**
`> [!NOTE]`、`> [!WARNING]` 等语法在微信中会被原样输出为文本。如需引用块，只用标准的 `>` 引用即可。

**2. 禁止在有序列表项中使用加粗开头 + 长文本**
`1. **加粗标题**：长内容...` 这种写法在微信端会导致编号错位和空行。替代方案：用加粗段落代替列表，每项之间用空行分隔。
```
❌ 错误写法：
1. **信息对齐**：你得知道老板在愁什么...
2. **节奏对齐**：在老板火力全开时...
...
**3. 禁止在正文 Markdown 中放工作附件**
SEO 备选标题、AI 提示词、视觉规划等元数据**不得**写在文章正文文件中。converter 会把整个文件转成 HTML，这些内容会直接出现在微信正文里。附件类信息应保存为独立文件（如 `{slug}-meta.md`）。
